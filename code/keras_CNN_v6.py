...
frac=0.1 #fraction of the dataset to train on
epochs=10
dataset='lines' #which dataset to train
...
def get_outputs(model, input_name, layer_name, X_batch): #defined here to grab layer shapes
...
FM1=5
ker1=5
pool1=1 #currently broken in nengo implementation, so keep value at 1
pad1=0 #no option for padding in convolution2d anyway
stride1=1 #currently broken in nengo implementation, so keep value at 1
...
#1st salience layer
size1=get_outputs(model,'input','maxpool0',X_train[:samples_train]).shape[-1] #shape of maxpool0
model.add_node(AveragePooling2D(pool_size=(size1,size1)),name='sal_f0', input='drop0') #salience F = avg of FM activation
...
model.add_node(Dense(FM3, activation='relu'), name='dense_sal_23', input='final_sal_f1')
...
#1st dense layer (classification)
model.add_node(Dense(classes, activation='softmax'), name='dense_C1', input='flat_conv')
model.add_node(Dense(classes, activation='softmax'), name='dense_S1', input='flat_sal')
...
#appologies about this. I wanted to automate this with model using keras export functions, but the weight information
#does not contain the custom_name attribute of their inputs, making it impossible to reconstruct an arbitrary network.
#After many hours searching online for a solution and an ignored post to Keras.usergroups, I put this hand-made export
#together. It must be manually modified when changing the model structure, or the Nengo import won't work.
arch_dict=OrderedDict((
	...
	('dense_C1', 		{'type':'Dense', 			'input_name':'flat_conv', 						'weights':model.nodes['dense_C1'].get_weights()[0],		'biases':model.nodes['dense_C1'].get_weights()[1], 		'activation':'softmax', 														}),
	('dense_S1', 		{'type':'Dense', 			'input_name':'flat_sal', 						'weights':model.nodes['dense_S1'].get_weights()[0],		'biases':model.nodes['dense_S1'].get_weights()[1], 		'activation':'softmax', 														}),
...
def get_activities(model): #for calculating layer statistics
	conv_nodes=[]
	avg_pool_nodes=[]
	max_pool_nodes=[]
...
def output_stuff(model, history):
	...
	# weights - can't effectively load into nengo using this method
...
def output_stats(filename,conv_nodes, avg_pool_nodes, max_pool_nodes):
	...
	for i in range(len(conv_nodes)):
		...
		#mean and std for each feature_map in that layer
...
#Print the error rate for comparison with Nengo (validation)
