...
for n in range(N):
	if np.mod(n,100)==0: print '%s/%s images generated' %(n,N) #track for long runs

	img=np.zeros((pixels_x, pixels_y))
	#0=(-), 1=(|), 2=(\), 3=(x), 4=(+)
	label=np.random.random_integers(low=0,high=2)

	#randomize line location and width
	...
	images.append(img)
	labels.append(label)
	# plt.imshow(img, cmap='gray',vmin=0, vmax=1) #visualize the image
	# plt.show()

np.savez_compressed('mix_data',np.array(images)) #save in compressed numpy array format
