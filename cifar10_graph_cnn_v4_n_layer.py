...
# Parameters
filename='cifar10_graph_cnn_v4_n_layer'
batch_size = 32
classes = 10
epochs = 10
learning_rate=0.01
...
pool_size = [3,3,3,3,3] # square size of pooling window at layer_i, len(n_conv_layers)
kernel_size = [9,7,5,3,1] # square size of kernel at layer_i, len(n_conv_layers)
...
def output_stats(filename,conv_nodes, avg_pool_nodes, max_pool_nodes):
	...
	for i in range(len(conv_nodes)):
		...
		mean_A_conv=str(np.average(A_conv, axis=(0,1,2)))
		std_A_conv=str(np.std(A_conv, axis=(0,1,2)))
		min_A_conv=str(np.min(A_conv, axis=(0,1,2)))
		max_A_conv=str(np.max(A_conv, axis=(0,1,2)))
		#stats across feature_maps in that layer
		std_across_features_mean_A_conv=str(np.std(np.average(A_conv,axis=(1,2))))
		std_across_features_max_A_conv=str(np.std(np.max(A_conv,axis=(1,2))))

		#stats of all avgpooled node activations (dxd subset, like superpixels)
		#in feature_map_0 of layer_i
		mean_acoss_map0_A_avg=str(np.average(A_avg[0], axis=(0,1)))
		std_acoss_map0_A_avg=str(np.std(A_avg[0], axis=(0,1)))
		min_acoss_map0_A_avg=str(np.min(A_avg[0], axis=(0,1)))
		max_acoss_map0_A_avg=str(np.max(A_avg[0], axis=(0,1)))
		...
		std_across_features_A_avg_centerunit=str(np.std(A_avg[:,center_x,center_y]))

		#stats of all maxpooled node activations in feature_map_0 of layer_i
		mean_acoss_map0_A_max=str(np.average(A_max[0], axis=(0,1)))
		std_acoss_map0_A_max=str(np.std(A_max[0], axis=(0,1)))
		min_acoss_map0_A_max=str(np.min(A_max[0], axis=(0,1)))
		max_acoss_map0_A_max=str(np.max(A_max[0], axis=(0,1)))
		#stats of avgpooled center node across feature_maps in layer_i
		center_x, center_y = A_max.shape[1]/2, A_max.shape[2]/2
		std_across_features_A_max_centerunit=str(np.std(A_max[:,center_x,center_y]))
		...
		writer.writerow(['min_A_conv',min_A_conv])
		writer.writerow(['max_A_conv',max_A_conv])
		writer.writerow(['std_across_features_mean_A_conv',std_across_features_mean_A_conv])
		writer.writerow(['std_across_features_max_A_conv',std_across_features_max_A_conv])

		writer.writerow(['mean_acoss_map0_A_avg',mean_acoss_map0_A_avg])
		writer.writerow(['std_acoss_map0_A_avg',std_acoss_map0_A_avg])
		writer.writerow(['min_acoss_map0_A_avg',min_acoss_map0_A_avg])
		writer.writerow(['max_acoss_map0_A_avg',max_acoss_map0_A_avg])
		writer.writerow(['std_across_features_A_avg_centerunit',std_across_features_A_avg_centerunit])

		writer.writerow(['mean_acoss_map0_A_max',mean_acoss_map0_A_max])
		writer.writerow(['std_acoss_map0_A_max',std_acoss_map0_A_max])
		writer.writerow(['min_acoss_map0_A_max',min_acoss_map0_A_max])
		writer.writerow(['max_acoss_map0_A_max',max_acoss_map0_A_max])
		writer.writerow(['std_across_features_A_max_centerunit',std_across_features_A_max_centerunit])
