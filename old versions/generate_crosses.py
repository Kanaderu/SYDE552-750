...
# SYDE 552/750
# Final Project
# Script to generate images with multiple lines, for input to nengo attention CNN
	...
	for i in range(pixels_x):
		for j in range(pixels_y):
			...
			if i>=np.min(line_c) and i <= np.max(line_c):
				img[j][i]=line_i
			# if (label == -1 and i==pixels_x-j) #gives X shape
			# 	img[j][i]=line_i
...
np.save('crosses_data',np.array(images))
