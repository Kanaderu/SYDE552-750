...
\section{Assignment: Linear-Nonlinear Poisson
Models}\label{assignment-linear-nonlinear-poisson-models}

\section{March 22, 2016}\label{march-22-2016}
...
Populating the interactive namespace from numpy and matplotlib
    ...
    \subsection{1. Spike Statistics}\label{spike-statistics}

    Generate 50 10-second trials of Poisson spikes at 25 spikes/s witha 5ms
absolute refractory period. To do this, draw samples from an
approprpiate ISI distribution.
...
\end{Verbatim}
    ...
    Plot the spike raster (1s) and the ISI histogram (10ms bins from
0-500ms)

    Calculate the coefficient of variation of the spike rate over all
trials.

    Calculate the Fano factor for the first 100ms. Comment on the
consistency of the Fano factor over multiple runs. Comment on the
difference in Fano factors with 5ms vs 1ms refractory period.
...
/usr/local/lib/python2.7/dist-packages/ipykernel/\_\_main\_\_.py:17: DeprecationWarning: using a non-integer number instead of an integer will result in an error in the future
    ...
    Calculate and plot the multi-trial firing rate of the synthetic neuron
(using methods from the previous assignment) over 250 repeated trials
with the same white noise stimulus. Overlay the prediction from the
linear poisson model.

    Scatterplot the multi-trial firing rate vs.~the linear poisson model
prediction. Fit the neural nonlinearity using
...
/usr/local/lib/python2.7/dist-packages/ipykernel/\_\_main\_\_.py:17: DeprecationWarning: using a non-integer number instead of an integer will result in an error in the future
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/usr/local/lib/python2.7/dist-packages/ipykernel/\_\_main\_\_.py:67: DeprecationWarning: using a non-integer number instead of an integer will result in an error in the future
/usr/local/lib/python2.7/dist-packages/ipykernel/\_\_main\_\_.py:64: RuntimeWarning: overflow encountered in exp
/usr/local/lib/python2.7/dist-packages/ipykernel/\_\_main\_\_.py:74: DeprecationWarning: using a non-integer number instead of an integer will result in an error in the future
    \end{Verbatim}
